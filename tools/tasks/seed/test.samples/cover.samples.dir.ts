...
export = () => {
  ...
  let cjsSpecFiles = [
    join(Config.SAMPLES_DIR, '**', '*[spec].js')
  ];
  ...
  return gulp.src(cjsSpecFiles)
    ...
    .pipe(plugins.istanbul.writeReports({
      ...
      reportOpts: { dir: Config.COVERAGE_BASE_SAMPLES_DIR }
    })).on('end', remapCoverageFiles); //remap

  //using remap-istanbul we can point our coverage data back to the original ts files
  function remapCoverageFiles() {
    return gulp.src(Config.COVERAGE_SAMPLES_JSON_PATH)
      .pipe(remapIstanbul({
        // basePath: './src',
        reports: {
          'html': Config.COVERAGE_BASE_SAMPLES_DIR,
          'text-summary': null
        }
      }));
  }
